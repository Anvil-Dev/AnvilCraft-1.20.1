...
import dev.dubhe.anvilcraft.client.gui.screen.inventory.JewelCraftingScreen;
...
public class ModMenuTypes {
    ...
    public static final MenuEntry<JewelCraftingMenu> JEWEL_CRAFTING = REGISTRATE
        .menu(
            "jewel_crafting",
            (type, id, inv) -> new JewelCraftingMenu(type, id, inv),
            () -> JewelCraftingScreen::new
        ).register();
    ...
    public static void open(ServerPlayer player, MenuProvider provider) {
        ModMenuTypesImpl.open(player, provider);
    }

    public static void open(ServerPlayer player, MenuProvider provider, BlockPos pos) {
        ModMenuTypesImpl.open(player, provider, pos);
    }
}
